...
{
	int i;
#pragma omp parallel for shared(n,weights,arrayX,arrayY,xr,yr) private(i)
	for(i = 0; i < n; i++){
...
#pragma omp parallel for private(i)
	  ...
	  arrayY[i] += -2 + 2*rand2(seed,i);
   	...
   	for(i = 0; i<n; i++){
		...
		//#pragma omp parallel for shared(Ones, probability, i) private(j, temp)
   		for(j = 0; j < Ones; j++) {
		  temp = array[index[i*Ones + j]];
   			probability[i] += (pow((temp - 100),2) -
   							  pow((temp-228),2))/50.0;
   		}
